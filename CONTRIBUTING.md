...
The mkdocs site uses fully pinned Python dependencies so the `mkdocs
build --strict` job is reproducible and `pip-audit` (CI) can attribute
each CVE to a known version.

- `requirements-docs.in` — human-edited top-level pins (`mkdocs`,
  `mkdocs-material`).
- `requirements-docs.txt` — auto-generated by `pip-compile`. Never edit
  by hand.

To bump a top-level pin:
...
Pin cadence: bump on a quarterly schedule (Jan / Apr / Jul / Oct) and
out-of-band whenever the docs CI `pip-audit` step flags a CVE.

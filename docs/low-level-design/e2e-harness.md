...
- **SUT lifecycle.** The orchestrator compiles the SUT (`go build ./cmd/fhir-subscriptions-foss`), generates a per-scenario config file pointing at the mocks and the Postgres container, starts the SUT as a child process, waits for `/readyz` to return green, and tears it down at scenario end. Logs from the SUT stream to a per-scenario file for the debug bundle.

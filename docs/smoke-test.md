...
This step runs on the deploy host (zdock or laptop running compose), not over the public hostname (Cloudflare's tunnel is HTTP-only — MLLP is LAN-only by design).

...
**Reader's prerequisites.** [overview.md](../../high-level-design/overview.md), [domains/ehr-adapter.md](../../high-level-design/domains/ehr-adapter.md), [contracts/adapter-spi.md](../../high-level-design/contracts/adapter-spi.md), [adapter-authoring-guide.md](../../adapter-authoring-guide.md). The Cerner, Athena, and eClinicalWorks scaffolds in [`adapters/cerner/cerner.go`](https://github.com/bzimbelman/fhir-ehr-subscriptions-service/blob/main/adapters/cerner/cerner.go), [`adapters/athena/athena.go`](https://github.com/bzimbelman/fhir-ehr-subscriptions-service/blob/main/adapters/athena/athena.go), and [`adapters/eclinicalworks/eclinicalworks.go`](https://github.com/bzimbelman/fhir-ehr-subscriptions-service/blob/main/adapters/eclinicalworks/eclinicalworks.go) show the manifest shape this adapter will follow. Current scaffold: [`adapters/meditech/meditech.go`](https://github.com/bzimbelman/fhir-ehr-subscriptions-service/blob/main/adapters/meditech/meditech.go).
...
**Recommendation:** ship the adapter with `VendorAPIClient: false` and `BuildVendorAPIClient` returning `nil`. After production validation against an Expanse tenant confirms Bulk Data semantics, add a `VendorAPIClient` implementation that polls `Group/$export` with `_since`, parallel to the Cerner adapter's design (see [`adapters/cerner/cerner.go`](https://github.com/bzimbelman/fhir-ehr-subscriptions-service/blob/main/adapters/cerner/cerner.go) and the Cerner HLD §3.3). This is a future enhancement, not a launch blocker.
...
- Current scaffold: [`adapters/meditech/meditech.go`](https://github.com/bzimbelman/fhir-ehr-subscriptions-service/blob/main/adapters/meditech/meditech.go)

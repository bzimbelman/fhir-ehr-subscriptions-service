...
| [0011](0011-message-channel-determinism.md) | Message-channel Bundle bytes are deterministic via typed-struct serialization + raw-message inner-entry passthrough + injectable Clock/NewID, not full JCS. | Byte-stable wrap output without paying the JCS tax at the wrong layer. | Determinism within the channel without re-shaping adapter-produced inner resources; coexists with JCS at the hash/signature seam (ADR 0010 §3). |

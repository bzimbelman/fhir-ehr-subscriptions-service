...
| 116 | HARDCODED-SECRET | HIGH | `demo/config.yaml:42` | `material: "bocf8u…"` (REDACTED — original 44-char base64 AES-256 key) was a real key checked into source. Anyone copying `demo/config.yaml` as a template encrypted PHI under a public key. RESOLVED by OP #155: key is now read from `${file:/etc/fhir-subs/secrets/at_rest_key}` and generated at first-time setup. |
...
**Path:** `demo/config.yaml:42` **Status:** RESOLVED by OP #155.

The literal 32-byte base64-encoded AES key (REDACTED — first six chars
`bocf8u…`) was committed to source. The bytes are intentionally NOT
reproduced here so a CI grep over the repo for the literal continues
to fail any reintroduction.

**Operator consequence.** Anyone copying `demo/config.yaml` as a
starting template into a real environment shipped PHI encrypted under
a public key.

**Resolution.** OP #155 replaced the literal with
`${file:/etc/fhir-subs/secrets/at_rest_key}`; the bridge interpolates
the file's bytes at config-load time. `demo/scripts/generate-keys.sh`
materialises the key on first-time setup; `demo/secrets/.gitignore`
keeps the generated bytes out of VCS. A repo-wide test in
`demo/config_cleanup_test.go` walks the tree and fails CI if the

...
| S-2.8 | `searchSubscriptions` no pagination | RESOLVED | story/47 | keyset cursor + `_count`; FHIR Bundle.link self/next |
...
| S-2.15 | `$events` replay hardcoded `LIMIT 1000` | RESOLVED | story/47 | `EventReplayPageSize` knob; Bundle.link next on truncation |
...
- **S-2.8 RESOLVED (story/47)** `internal/api/handlers/subscription_handlers.go:431-545` — `searchSubscriptions` is now keyset-paginated by (created_at DESC, id DESC). New `Deps.SearchPageSize` (default 50) / `Deps.SearchMaxPageSize` (default 200) bound the page; the `_count` query parameter clamps to the cap silently per FHIR R5 §3.1.1.5. The `_cursor` query parameter is an opaque base64url(`<unix-nanos>:<uuid>`) round-tripped from the previous page's last-row tuple. Bundle.link emits `self` (always) and `next` (only when more rows exist). New `SubscriptionsStore.ListByClientPage` adds the storage seam; the legacy unbounded `ListByClient` remains for `matchingSubscriptions` and the admin surface.
...
- **S-2.15 RESOLVED (story/47)** `internal/api/handlers/pg_stores.go:264-314` — the legacy hardcoded `LIMIT 1000` is replaced by `Deps.EventReplayPageSize` (default 1000). The handler asks for `limit+1` rows so it can detect truncation in one round-trip; on truncation the response carries a Bundle.link `next` whose `eventsSinceNumber` is one past the last delivered event. New `EhrEventsStore.ListByTopicAndRangePage` adds the page surface alongside the legacy `ListByTopicAndRange`.

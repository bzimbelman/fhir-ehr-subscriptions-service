...
- **SHOULD-FIX (S-*):** 125 total — 109 RESOLVED, 4 PARTIAL, 0 OPEN, 12 DEFERRED
- **NICE-TO-HAVE (N-* / D-*):** 39 total — 32 RESOLVED, 0 PARTIAL, 4 OPEN, 3 DEFERRED
- **TOTAL:** 199 findings — 175 RESOLVED, 4 PARTIAL, 4 OPEN, 16 DEFERRED (88% closed)
...
| S-3.3 | No per-client rate limit on subscription create / WS bind-token | RESOLVED | `4766bbd` | `auth.ClientRateLimiter` middleware on POST /Subscription + $get-ws-binding-token; principal-ID-keyed bucket; 429 + Retry-After |
...
| S-9.6 | `ReaperBatchSize`, claim/idle knobs | RESOLVED | `acd798d`, story/61 | `ReaperBatchSize` + claim/idle knobs all exposed; counter parity confirmed under story/61 (`MetricClaimCycleErrors`, `MetricSameKindCollision`, and `MetricHL7TxBeginFailed` from story/53 all wired at call sites) |
...
| S-10.6 | `MaxRowAttempts` knob + counter | RESOLVED | `d3fad44`, story/61 | `MaxRowAttempts` knob (default 8) on `matcher.Config`; story/61 adds `fhir_subs_matcher_row_attempts_total{outcome}` registered in the host inventory and emitted from every `tickOnce` exit through the new `MetricsEmitter.RowAttempt` seam (mirrors `ResourceChangeClaimed` for cross-pipeline `row_attempts_total` aggregation); pre-seeded with `processed`/`deferred`/`error` outcomes so dashboards see all three series at scrape time. Dead-letter-at-cap behavior remains tracked under the storage refactor (no `attempt_count` column on `resource_changes` yet). |
...
| S-11.4 | Topic catalog Prometheus metrics | RESOLVED | (B-25 work), story/61 | `Catalog.Rejected()` / `Catalog.Overridden()` exposed (B-25); story/61 registers `fhir_subs_topics_rejected_total{origin,reason}` and `fhir_subs_topic_overridden_total{from,to}` in the host inventory, with `ObservabilityModule.PublishTopicCatalogReport(report)` translating each load's rejected / overridden entries into Inc calls. Pre-seeded with bounded `origin` enum (`builtin`, `adapter`, `operator`). |
...
| S-12.3 | submatcher `MaxRowAttempts` | RESOLVED | `d3fad44`, story/61 | `MaxRowAttempts` knob (default 8) on `submatcher.Config`; story/61 adds `fhir_subs_submatcher_row_attempts_total{outcome}` registered in the host inventory + a `Metrics.RowAttempt` seam emitted from every `tickOnce` exit (parallel to the matcher form). The `SubmatcherMetricsAdapter` exposed by the observability module forwards the call into the inventory; pre-seeded with `processed`/`deferred`/`error` outcomes. Dead-letter-at-cap behavior remains tracked under the storage refactor. |
| S-12.4 | Fanout tx inline `events_since_subscription_start` UPDATE | RESOLVED | story/56 | submatcher fanoutOne now accumulates `(subscription_id, event_number)` pairs per Match and flushes them in a single `UPDATE subscriptions ... FROM unnest($1::uuid[], $2::bigint[])` statement; `Config.CursorAdvanceBatchSize` (default 1000) bounds each batched UPDATE under Postgres' 65535 bind-parameter ceiling; pgxmock unit tests assert the batched form for N=64 and ceil(N/cap)=3 batches at N=250/cap=100; benchmark shows ~990x throughput improvement at 100µs per-Exec round-trip on a 1000-subscriber topic |
...
| S-12.9 | `fhir+xml` rejection at builder, not API | RESOLVED | `8642b71` | rejected at POST/PUT /Subscription with 400 OperationOutcome citing `fhir+xml`; builder hardcode kept as defense-in-depth |
...
| N-1.25 | MLLP no PROXY protocol v2 | RESOLVED | story/60 | Hand-rolled v2 parser + per-endpoint `proxy_protocol_v2` config knob; real client IP surfaced for B-19 admission and audit; `fhir_subs_mllp_proxy_header_rejected_total` counter for missing/spoofed headers |
...
#### S-3: Auth — RESOLVED in merge `1078e41` (S-3.3 follow-up RESOLVED in `4766bbd`)
...
- **`internal/api/handlers/subscription_handlers.go`** (POST /Subscription, $get-ws-binding-token) — no per-client rate limit on subscription creates / WS binding token mints. — **RESOLVED** (`4766bbd`) — `auth.ClientRateLimiter` middleware (new file `internal/api/auth/handler_rate_limit.go`) wraps both routes via `chi.With`, keyed on the authenticated principal's ClientID (falls back to source IP if no principal). Returns 429 + integer-second `Retry-After` on bucket exhaustion. Limits sourced from operator config (`auth.subscription_create_rate_limit.{burst,refill_per_second,max_keys}` and `auth.ws_binding_token_rate_limit.*`) and surfaced through `handlers.Deps.SubscriptionCreateRateLimit` / `WSBindingTokenRateLimit`. e2e: `api_per_client_rate_limit_test.go`; unit tests cover under/at/over-limit, per-client isolation, refill-after-window, and disabled (nil) pass-through in both `internal/api/auth/handler_rate_limit_test.go` and `internal/api/handlers/rate_limit_test.go`.
...
#### S-12: Engine / submatcher / builder — RESOLVED in merge `c1c6b32` + story/57 (2 sub-bullets DEFERRED — repo pagination + fanout batching; API-layer fhir+xml rejection landed in `8642b71`)
...
- **`internal/engine/submatcher/worker.go:302-310`** — RESOLVED in story/56: the per-match inline `UPDATE subscriptions SET events_since_subscription_start = GREATEST(...)` is gone. `fanoutOne` now accumulates `(subscription_id, event_number)` pairs into two parallel slices while streaming, and flushes them via `batchedAdvanceCursor` — a single `UPDATE subscriptions SET events_since_subscription_start = GREATEST(...) FROM unnest($1::uuid[], $2::bigint[]) AS batch(id, event_number) WHERE subscriptions.id = batch.id`. `Config.CursorAdvanceBatchSize` (default 1000) caps each flush so a hot topic stays under Postgres' 65535 bind-parameter ceiling on extended-protocol writes. pgxmock unit tests pin the batched shape (one UPDATE for N=64; three for N=250 at cap=100). A latency-aware micro-benchmark (`BenchmarkCursorAdvance_PerRow_vs_Batched`) at 100µs/Exec shows ~990x throughput improvement on a 1000-subscriber topic — comfortably above the 10x acceptance criterion.
...
- **`internal/engine/builder/builder.go:36-38`** — RESOLVED in `8642b71` (story/57): `application/fhir+xml` is rejected up front in `internal/api/handlers/subscription_handlers.go::requestsFHIRXML` for both POST and PUT, with HTTP 400 + an OperationOutcome (issue.code = `not-supported`) whose diagnostics quote `fhir+xml` and point at `docs/future-work.md` P2. The check looks at both wire shapes (R5 top-level `contentType` and R4B Backport `channel.payload`). The builder package's docstring is updated to point at the new gate; its `contentType` hardcode to `application/fhir+json` is now a defense-in-depth assertion since the API never lets a fhir+xml envelope reach Build. Validation failure counter `subscription.validation_failures{kind="unsupported_content_type"}` fires on rejection.
...
- **RESOLVED (story/60)** `internal/mllp/endpoint.go:101` — N-1.25 PROXY protocol v2 support landed as a per-endpoint opt-in (`EndpointConfig.ProxyProtocolV2`, surfaced as `mllp.listeners[].proxy_protocol_v2` in operator config). The accept loop reads a 16-byte v2 header (signature + ver/cmd + family/transport + length) before the admission check, parses IPv4 / IPv6 / LOCAL frames, and replaces `RemoteAddr` via a thin `proxiedConn` wrapper so per-IP caps (B-19) and audit logging see the real EHR rather than the upstream LB. v1 ASCII headers, non-v2 versions, truncated reads, and unsupported families are rejected before any HL7 bytes flow and counted under `fhir_subs_mllp_proxy_header_rejected_total{reason}`. Hand-rolled (no `pires/go-proxyproto` dep) — the wire format is small and a hand-rolled parser keeps the dependency surface lean. Tests: 7 unit-level cases in `internal/mllp/proxyproto_test.go` (IPv4 / IPv6 / LOCAL / TLV trailer / bad signature / truncated / non-v2) + 3 real-listener integration tests (enabled+sent surfaces real IP; enabled+missing rejects with metric; disabled+sent does NOT silently parse).
...
- ~~**N-1 → follow-up: PROXY protocol v2 in MLLP listener**~~ — RESOLVED on `story/60-mllp-proxy-proto-v2`. Hand-rolled v2 parser in `internal/mllp/proxyproto.go`; per-endpoint `proxy_protocol_v2` config knob; `fhir_subs_mllp_proxy_header_rejected_total{reason}` counter for missing/spoofed/non-v2 headers. No new external dependency taken.

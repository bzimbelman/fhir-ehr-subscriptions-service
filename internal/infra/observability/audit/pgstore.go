...
// Migrate is a no-op kept for backwards compatibility with callers that
// drove a pre-migration smoke setup. The audit_log schema is owned by
// migrations/0001_init.sql + migrations/0007_audit_log_chain_columns.sql
// (story #106): pgstore must NOT duplicate the DDL because the
// duplicate version drifted out of sync (it generated `seq BIGINT
// GENERATED BY DEFAULT AS IDENTITY` while production had BIGSERIAL,
// and it would silently re-create a wrong-schema table on a fresh
// schema search_path). Operators should run the migration suite via
// internal/infra/storage/migrate; ad-hoc tests that need a transient
// schema can call ApplyMigrations against the embedded files.
...
//
// Both row.OccurredAt and row.CorrelationID are passed through verbatim:
// they were hashed into row.ChainInput by the writer, and any server-
// side substitution (server clock for occurred_at, fresh uuid for a
// zero correlation_id) would diverge the on-disk row from the bytes
// the chain_hash was computed over (story #107). If a caller emits an
// event with a zero CorrelationID the zero UUID is what lands on disk;
// the writer's responsibility (or the caller above the writer) is to
// assign a real correlation id BEFORE Emit so the on-disk and hashed

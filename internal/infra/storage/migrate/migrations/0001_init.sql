...
create index subscription_topics_status_idx on subscription_topics (status);
create index subscription_topics_url_idx on subscription_topics (url);
...
create index subscriptions_topic_status_idx on subscriptions (topic_url, status);

comment on table subscriptions is 'Active and historical FHIR R5 Subscriptions registered by clients.';

create table hl7_message_queue (
    id uuid primary key default gen_random_uuid(),
...
create index hl7_message_queue_processed_received_idx on hl7_message_queue (processed, received_at);
...
create index pending_pairs_expires_at_idx on pending_pairs (expires_at);
...
create index resource_changes_sequence_idx on resource_changes (sequence);
create index resource_changes_processed_created_idx on resource_changes (processed, created_at);
...
create index ehr_events_topic_processed_idx on ehr_events (topic_url, processed);
create index ehr_events_event_number_idx on ehr_events (event_number);
...
create index dead_letters_kind_created_idx on dead_letters (kind, created_at);
...
create table ws_binding_tokens (
...
create index ws_binding_tokens_expires_at_idx on ws_binding_tokens (expires_at);
